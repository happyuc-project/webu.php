...
$webu->huc->accounts(function ($err, $accounts) use ($contract, $testBytecode) {
    if ($err === null) {
        ...
        $contract->bytecode($testBytecode)->new(1000000, 'Game Token', 1, 'GT', [
            ...
            $contract->huc->getTransactionReceipt($transactionId, function ($err, $transaction) use ($contract, $fromAccount, $toAccount) {
                if ($err !== null) {
                    throw $err;
                }
                if ($transaction) {
                    ...
                    $contract->at($contractAddress)->send('transfer', $toAccount, 16, [
                        ...
                        $contract->huc->getTransactionReceipt($transactionId, function ($err, $transaction) use ($fromAccount, $toAccount) {
                            if ($err !== null) {
                                throw $err;
                            }
                            if ($transaction) {
